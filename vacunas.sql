...
CREATE TABLE Provincia (
	...
	CONSTRAINT CK_Provincia_existe CHECK (provincia IN ('Provincia por registrar', /* Problemas o nueva provincia sin registrar aún */
												'Bocas del Toro', /*1*/
												'Coclé', /*2*/
												'Colón', /*3*/
												'Chiriquí', /*4*/
...
CREATE TABLE Usuarios (
	...
	CONSTRAINT CK_Usuarios_TipoValido CHECK (tipo in (1, 2, 3, 4, 5, 6, 7))
	/*  1 paciente
		2 doctor
...
BEGIN
    ...
    WHERE fecha_nacimiento IS NOT NULL AND
          (edad_calculada IS NULL OR
           IIF(DATEADD(YEAR, DATEDIFF(YEAR, fecha_nacimiento, GETDATE()), fecha_nacimiento) > GETDATE(),
               DATEDIFF(YEAR, fecha_nacimiento, GETDATE()) - 1, DATEDIFF(YEAR, fecha_nacimiento, GETDATE())) <> edad_calculada);
END;
GO
...
BEGIN
	...
	IF (@DependenciaSede NOT LIKE 'CSS' AND @DependenciaSede NOT LIKE 'MINSA' AND @DependenciaSede NOT LIKE 'Privada' AND @DependenciaSede NOT LIKE 'Por registrar')
	BEGIN
...
END;
GO
CREATE PROCEDURE ManipularPaciente
...
BEGIN
    ...
    IF (@DireccionResidencialPaciente IS NOT NULL AND @DistritoResidePaciente IS NULL) OR (@DireccionResidencialPaciente IS NULL AND @DistritoResidePaciente IS NOT NULL)
    BEGIN
        RAISERROR('Debe especificar ambos campos: dirección y distrito o ninguno.', 16, 1);
        RETURN;
    END
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN
                -- Insertar nueva dirección
                INSERT INTO Direccion (direccion, idDistrito)
                VALUES (@DireccionResidencialPaciente, (SELECT ID_distrito FROM Distrito WHERE distrito = @DistritoResidePaciente));
...
GO
*/
CREATE PROCEDURE InsertarDosis
    @cedulaPaciente VARCHAR(20),
    @fecha_aplicacion DATETIME,
    @numero_dosis CHAR(2),
    @idVacuna INT,
...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM Paciente_Dosis pd INNER JOIN Dosis d ON pd.idDosis = d.ID_dosis WHERE pd.cedulaPac = @CedulaPaciente AND d.idVacuna = @idVacuna AND d.numero_dosis = @numero_dosis)
	BEGIN
    ...
    BEGIN
        IF @numero_dosis = '2' AND NOT EXISTS (SELECT 1 FROM Paciente_Dosis pd JOIN Dosis d ON pd.idDosis = d.ID_dosis WHERE pd.cedulaPac = @cedulaPaciente AND d.numero_dosis = '1')
        BEGIN
            RAISERROR('La dosis 1 debe ser aplicada antes de la dosis 2.', 16, 1);
            RETURN;
        END

        IF @numero_dosis = '3' AND NOT EXISTS (SELECT 1 FROM Paciente_Dosis pd JOIN Dosis d ON pd.idDosis = d.ID_dosis WHERE pd.cedulaPac = @cedulaPaciente AND d.numero_dosis = '1')
        BEGIN
            RAISERROR('La dosis 1 debe ser aplicada antes de la dosis 3.', 16, 1);
            RETURN;
        END

        IF @numero_dosis = 'R' AND NOT EXISTS (SELECT 1 FROM Paciente_Dosis pd JOIN Dosis d ON pd.idDosis = d.ID_dosis WHERE pd.cedulaPac = @cedulaPaciente AND d.numero_dosis = '1')
        BEGIN
        ...
        IF @numero_dosis = 'R1' AND NOT EXISTS (SELECT 1 FROM Paciente_Dosis pd JOIN Dosis d ON pd.idDosis = d.ID_dosis WHERE pd.cedulaPac = @cedulaPaciente AND d.numero_dosis IN ('1', 'R'))
        BEGIN
            RAISERROR('La dosis 1 o R debe ser aplicada antes de la dosis R1.', 16, 1);
            RETURN;
        END

        IF @numero_dosis = 'R2' AND NOT EXISTS (SELECT 1 FROM Paciente_Dosis pd JOIN Dosis d ON pd.idDosis = d.ID_dosis WHERE pd.cedulaPac = @cedulaPaciente AND d.numero_dosis IN ('R1', '2'))
        BEGIN
            RAISERROR('La dosis R1 o 2 debe ser aplicada antes de la dosis R2.', 16, 1);
            RETURN;
        END
    END
	ELSE IF @numero_dosis LIKE 'P'
    BEGIN
        IF EXISTS (SELECT 1 FROM Paciente_Dosis pd
    ...
    WHERE PD.cedulaPac = @cedulaPaciente AND D.idVacuna = @idVacuna;
    ...
    IF @numero_dosis = '2' AND @ultima_dosis IS NOT NULL AND DATEDIFF(MONTH, @ultima_dosis, @fecha_aplicacion) < @intervalo_dosis
    BEGIN
	...
	BEGIN TRY
		...
		DECLARE @idDosis INT;
		SET @idDosis = SCOPE_IDENTITY(); -- Obtener el ID de la dosis insertada
...
END;
GO
CREATE PROCEDURE DistribuirVacunas
    @ID_almacen INT,
    @ID_sede INT,
    @ID_vacuna INT,
    @Cantidad INT,
...
AS
BEGIN
	IF NOT EXISTS (SELECT 1 FROM Almacen_Inventario WHERE ID_almacen = @ID_almacen AND ID_vacuna = @ID_vacuna AND lote_almacen LIKE @Lote)
	BEGIN
    ...
    BEGIN TRANSACTION;
        ...
        IF EXISTS (SELECT 1 FROM Sede_Inventario WHERE ID_sede = @ID_sede AND ID_vacuna = @ID_vacuna AND lote_sede LIKE @Lote)
			BEGIN
				UPDATE Sede_Inventario
				SET cantidad = cantidad + @Cantidad
    ...
    BEGIN TRANSACTION
        ...
        BEGIN
            ...
            SET usuario = @Usuario,
                clave_hash = @Clave,
                tipo = @Tipo
            WHERE cedula = @Cedula AND tipo = @Tipo;
        END
        ELSE
        BEGIN
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM Paciente WHERE cedula = @cedulaPaciente)
    BEGIN
        SELECT @result = 'Paciente encontrado: ' + nombre_paciente + ' ' + apellido_paciente + ', Fecha de nacimiento: ' + CONVERT(VARCHAR, fecha_nacimiento, 103)
        FROM Paciente
        WHERE cedula = @cedulaPaciente;
    END
    ELSE
...
(
    ...
    WHERE
        (f.ID_fabricante = @idFabricante OR @idFabricante IS NULL)
        AND
        (fv.ID_vacuna = @idVacuna OR @idVacuna IS NULL)
);
GO
...
(
    ...
    WHERE
        (d.idVacuna = @idVacuna OR @idVacuna IS NULL)
        AND
        (pd.cedulaPac = @cedulaPac OR @cedulaPac IS NULL)
        AND
        (d.numero_dosis = @numeroDosis OR @numeroDosis IS NULL)
        AND
        (d.fecha_aplicacion = @fechaAplicacion OR @fechaAplicacion IS NULL)
        AND
        (d.idSede = @idSede OR @idSede IS NULL)
);
GO
-- del app usuarios
...
RETURN
(
	SELECT cedula, usuario, tipo, fecha_nacimiento, created_at
	FROM Usuarios
	where cedula = @cedula and tipo = @tipo
);
GO
...
-- Usuario para inicializacion Java
CREATE USER AplicacionJava FOR LOGIN ApplicationJava
GRANT SELECT ON Paciente TO AplicacionJava
GRANT SELECT ON BuscarUsuario TO AplicacionJava
GRANT SELECT, UPDATE, DELETE, INSERT ON Usuarios TO AplicacionJava
GRANT SELECT ON Direccion TO AplicacionJava
GRANT SELECT ON Distrito TO AplicacionJava
GRANT EXECUTE ON CrearUsuario TO AplicacionJava
GRANT EXECUTE ON ManipularPaciente TO AplicacionJava
GO
...
GRANT SELECT ON Usuarios TO Fabricante -- TODO Investigando
GRANT SELECT ON BuscarUsuario TO Fabricante -- TODO Investigando
GRANT SELECT ON BuscarFabricante TO Fabricante
-- permitido insertar sus vacunas
GRANT INSERT ON Fabricante_Vacuna TO Fabricante
GO
CREATE USER Doctor FOR LOGIN Doctores
--Doc
GRANT SELECT ON Distrito TO Doctor
...
GRANT SELECT ON Sede TO Doctor
...
GRANT SELECT ON Usuarios TO Doctor -- TODO Investigando
GRANT SELECT ON BuscarUsuario TO Doctor -- TODO Investigando
GRANT SELECT ON BuscarDosis TO Doctor
...
GRANT UPDATE ON Usuarios TO Doctor
...
GRANT EXECUTE ON CrearUsuario TO Doctor
GRANT EXECUTE ON ManipularPaciente TO Doctor
GRANT EXECUTE ON InsertarDosis TO Doctor
GRANT EXECUTE ON BuscarPacientePorCedula TO Doctor
GO
...
GRANT SELECT ON Usuarios TO Paciente -- TODO Investigando
GRANT SELECT ON BuscarUsuario TO Paciente -- TODO Investigando
-- permitido utilizar procedimientos
GRANT EXECUTE ON ActualizarEdadPacientes TO Paciente
GRANT EXECUTE ON BuscarPacientePorCedula TO Paciente
GO
...
GRANT SELECT ON Usuarios TO Oficinista -- TODO Investigando
GRANT SELECT ON BuscarUsuario TO Oficinista -- TODO Investigando
...
GRANT EXECUTE ON BuscarPacientePorCedula TO Oficinista
GO
...
EXEC dbo.ManipularPaciente '8-1006-14', 'Jorge', 'Ruiz', '1999-05-31', 'M', '507 6068-4595', NULL, 'Samaria, sector 4, casa E1', 'San Miguelito';
GO
EXEC dbo.ManipularPaciente '1-54-9635', 'Luis', 'Mendoza', '2006-05-05', 'M', '507 6325-7865', NULL, 'Finca 30, casa 45', 'Changuinola';
GO
EXEC dbo.ManipularPaciente '2-4558-5479', 'José', 'Perez', '1959-12-13', 'M', '507 6265-4789', NULL, 'Vía Interamericana, casa L78', 'Natá';
GO
EXEC dbo.ManipularPaciente '5-554-321', 'Martha', 'Cornejo', '1979-08-24', 'F', '507 6784-1296', NULL, 'Vía Interamericana, Metetí, casa 87F', 'Pinogana';
GO
EXEC dbo.ManipularPaciente '8-9754-1236', 'Suleimi', 'Rodriguez', '2001-02-17', 'F', '507 6785-9631', NULL, 'Calle 51, casa 74', 'Panamá';
GO
-- insertar sede con procedimiento
EXEC dbo.InsertarSede 'Sede por registrar', 'Por registrar', NULL, NULL, NULL, NULL;
GO
EXEC dbo.InsertarSede 'Hospital San Miguel Arcangel', 'MINSA', NULL, '507 523-6906', 'HISMA, Vía Ricardo J. Alfaro', 'San Miguelito';
GO
...
GO
EXEC dbo.InsertarSede 'POLICENTRO DE SALUD de Chepo', 'MINSA', NULL, '507 296-7220', 'Via Panamericana Las Margaritas', 'Chepo';
GO
EXEC dbo.InsertarSede 'Clínica Hospital San Fernando', 'Privada', NULL, '507 305-6300', 'Via España, Hospital San Fernando', 'Panamá';
GO
EXEC dbo.InsertarSede 'Complejo Hospitalario Doctor Arnulfo Arias Madrid', 'CSS', NULL, '507 503-6032', 'Avenida José de Fábrega, Complejo Hospitalario', 'Panamá';
GO
EXEC dbo.InsertarSede 'Hospital Santo Tomás', 'MINSA', NULL, '507 507-5830', 'Avenida Balboa y Calle 37 Este', 'Panamá';
GO
EXEC dbo.InsertarSede 'Hospital Regional de Changuinola Raul Dávila Mena', 'MINSA', NULL, '507 758-8295', 'Changuinola, Bocas del Toro', 'Changuinola';
GO
EXEC dbo.InsertarSede 'Hospital Dr. Rafael Hernández', 'MINSA', NULL, '507 774-8400', 'David, Chiriquí', 'David';
GO
EXEC dbo.InsertarSede 'Hospital Punta Pacífica Pacífica Salud', 'Privada', NULL, '507 204-8000', 'Punta Pacífica, Ciudad de Panamá', 'Panamá';
GO
...
GO
EXEC dbo.InsertarSede 'Hospital Dr. Nicolás A. Solano', 'MINSA', NULL, '507 254-8926', 'La Chorrera, Panamá Oeste', 'La Chorrera';
GO
EXEC dbo.InsertarSede 'Cómplejo Hospitalario Dr. Manuel Amador Guerrero', 'CSS', NULL, '507 475-2207', 'Colón, Colón', 'Colón';
GO
EXEC dbo.InsertarSede 'Policlínica Lic. Manuel María Valdés', 'CSS', NULL, '507 503-1500', 'San Miguelito, Ciudad de Panamá', 'Panamá';
GO
EXEC dbo.InsertarSede 'CSS Complejo Metropolitano', 'CSS', NULL, '507 506-4000', 'Vía España, Ciudad de Panamá', 'Panamá';
GO
EXEC dbo.InsertarSede 'Hospital de Especialidades Pediátricas Omar Torrijos Herrena', 'CSS', NULL, '507 513-7008', 'Vía España, Ciudad de Panamá', 'Panamá';
GO
INSERT INTO Vacuna (nombre_vacuna, edad_minima, intervalo_dosis_1_2_meses) VALUES
('Vacuna por registrar', NULL, NULL),
('Adacel', 132, NULL),
('BCG', 0, NULL),
('COVID-19', 6, 0.92),
...
INSERT INTO Fabricante (nombre_fabricante, telefono_fabricante, correo_electronico_fabricante, direccion_fabricante, contacto_fabricante)
VALUES -- algunos datos no son veraces
('Sanofi Pasteur', '1-800-822-2463', 'info@sanofipasteur.com', 'Sanofi Pasteur Inc., 1 Discovery Drive, Swiftwater, PA 18370, USA', 'John Doe'),
('Pfizer', '1-212-733-2323', 'support@pfizer.com', 'Pfizer Inc., 235 East 42nd Street, New York, NY 10017, USA', 'Alice Johnson'),
('GlaxoSmithKline', '44-20-8047-5000', 'info@gsk.com', 'GSK plc, 980 Great West Road, Brentford, Middlesex, TW8 9GS, UK', 'Bob Brown'),
('Merck', '1-908-740-4000', 'contact@merck.com', 'Merck & Co., Inc., 2000 Galloping Hill Road, Kenilworth, NJ 07033, USA', 'Jane Smith'),
('Serum Institute', '91-20-26993900', 'contact@seruminstitute.com', '212/2, Hadapsar, Off Soli Poonawalla Road, Pune 411028, Maharashtra, India', 'Mr. Muralidharan Poduval');
GO
INSERT INTO Fabricante_Vacuna (ID_fabricante, ID_vacuna) VALUES
(1, 1),  -- Adacel - Sanofi Pasteur
...
VALUES
(1, 1, 160, '2024-12-15', 'LoteA1'),
(1, 2, 215, '2024-12-16', 'LoteA2'),
(1, 3, 140, '2024-12-17', 'LoteA3'),
(1, 4, 325, '2024-12-18', 'LoteA4'),
(1, 5, 280, '2024-12-19', 'LoteA5'),
(1, 6, 215, '2024-12-20', 'LoteA6'),
(1, 7, 260, '2024-12-21', 'LoteA7'),
(1, 8, 235, '2024-12-22', 'LoteA8'),
(1, 9, 190, '2024-12-23', 'LoteA9'),
(1, 10, 185, '2024-12-24', 'LoteA10'),
(1, 11, 170, '2024-12-25', 'LoteA11'),
(1, 12, 235, '2024-12-26', 'LoteA12'),
(1, 13, 230, '2024-12-27', 'LoteA13'),
(1, 14, 165, '2024-12-28', 'LoteA14'),
(1, 15, 160, '2024-12-29', 'LoteA15'),
(1, 16, 155, '2024-12-30', 'LoteA16'),
(1, 17, 150, '2024-12-31', 'LoteA17'),
(1, 18, 145, '2024-12-01', 'LoteA18'),
(1, 19, 140, '2024-12-02', 'LoteA19'),
(1, 20, 135, '2024-12-03', 'LoteA20'),
(1, 21, 130, '2024-12-04', 'LoteA21'),
(1, 22, 125, '2024-12-05', 'LoteA22'),
(1, 23, 125, '2024-12-06', 'LoteA23');
GO
...
GO
EXEC dbo.InsertarDosis '1-54-9635', '2024-07-04 10:00:00', '2', 2, 2, 'LoteA2'; -- BCG en MINSA CAPSI FINCA 30 CHANGINOLA
GO
EXEC dbo.InsertarDosis '2-4558-5479', '2024-01-03 08:35', '1', 7, 7, 'LoteA7'; -- Hep B en Complejo Hospitalario Doctor Arnulfo Arias Madrid
GO
...
GO
EXEC dbo.InsertarDosis '8-9754-1236', '2022-10-03 16:00', '1', 18, 18, 'LoteA18'; -- Priorix en Hospital Manuel Amador Guerrero
GO
EXEC dbo.InsertarDosis '8-9754-1236', '2023-11-30 07:00', '2', 18, 18, 'LoteA18'; -- Priorix en Hospital Manuel Amador Guerrero
GO
